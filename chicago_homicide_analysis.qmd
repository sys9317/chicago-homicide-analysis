...
The full "Crimes 2001 to Present" dataset holds over 8 million records. Rather than download the whole ~2&nbsp;GB file, this analysis pulls **only homicides from the last 10 years** directly from the Chicago Data Portal (Socrata API, dataset `ijzp-q8t2`), so the report is reproducible from a clean clone. If a local copy of the bulk CSV is present in `data/`, it is used instead.

```{r}
#| label: load-crimes
#| code-summary: "Load Chicago homicide data (Socrata API, or local bulk CSV if present)"

local_csv <- "data/Crimes_-_2001_to_Present_20251020.csv"

if (file.exists(local_csv)) {
  crimes <- read_csv(
    local_csv,
    col_types = cols(Longitude = col_character(), Latitude = col_character())
  )
} else {
  cutoff <- format(Sys.Date() - lubridate::years(10), "%Y-%m-%dT00:00:00")
  where  <- utils::URLencode(
    sprintf("primary_type='HOMICIDE' AND date > '%s'", cutoff),
    reserved = TRUE
  )
  api_url <- sprintf(
    "https://data.cityofchicago.org/resource/ijzp-q8t2.csv?$where=%s&$limit=100000",
    where
  )
  crimes <- read_csv(api_url, show_col_types = FALSE)
}
...
crimes_lim <- crimes |>
  # `date` is "MM/DD/YYYY hh:mm:ss AM/PM" in the bulk CSV and ISO 8601 from the API
  mutate(
    date = date |>
      str_replace("T", " ") |>
      str_remove("\\.\\d+$") |>
      lubridate::parse_date_time(orders = c("m/d/Y I:M:S p", "Y-m-d H:M:S"))
  ) |>
  filter(
...
We load the Chicago census tract shapefile now so it can serve as a basemap for the point map below, and for the tract-level aggregation that follows.
...
This map shows every homicide incident, color-coded by whether an arrest was made. Red = no arrest; blue = arrest.
...
crimes_lim |>
  ggplot() +
  geom_sf(data = chicago_tracts, fill = "grey96", color = "grey85", linewidth = 0.1) +
...
To analyze neighborhood-level patterns, we aggregate homicides by census tract, using the boundaries loaded above.
...
chicago_analysis |>
  mutate(homicide_count = tidyr::replace_na(homicide_count, 0)) |>
  ggplot() +
...
chicago_analysis |>
  # keep every tract in the map, but only shade arrest rate where the tract has
  # enough homicides (>= 3) for the rate to be meaningful; the rest render grey
  mutate(arrest_rate = if_else(coalesce(homicide_count, 0L) >= 3, arrest_rate, NA_real_)) |>
  ggplot() +
  ...
  labs(
    ...
    subtitle = "Tracts with fewer than 3 homicides shown in grey",
...
if (!nzchar(census_key)) {
  stop("CENSUS_API_KEY is not set. Get a free key at https://api.census.gov/data/key_signup.html, ",
       "click the activation link in the confirmation email, then put it in a .env file: ",
...
# The Census API redirects an unactivated / invalid key to an HTML error page,
# so guard the JSON parse and fall back to tidycensus for a transient hiccup.
census_data <- tryCatch({
  if (http_error(response) || !startsWith(trimws(raw_data), "[")) {
    stop("Census API did not return JSON (HTTP ", status_code(response),
         "). Most often this means the API key has not been activated yet.")
  }
  dm  <- fromJSON(raw_data)
  out <- as_tibble(dm[2:nrow(dm), ], .name_repair = "minimal")
  names(out) <- dm[1, ]
  out |>
    rename(
      tract_name      = NAME,
  ...
  message("Raw API call failed (", conditionMessage(e), "); trying tidycensus::get_acs()...")
  tryCatch(
    get_acs(
      ...
      transmute(
        ...
        bachelor_degree = bachelor_degreeE,
        poverty_count   = poverty_countE
      ),
    error = function(e2) stop(
      "Both the raw Census API and tidycensus failed. This almost always means the ",
      "CENSUS_API_KEY has not been activated yet: check your email from the U.S. Census ",
      "Bureau and click the activation link, wait a few minutes, then re-render. (",
      conditionMessage(e2), ")"
    )
  )
})
